...
<body>
    ...
    <% if (isFaved) { %>
        <p>You've favorited this pokemon!</p>
    <% } else { %>
        <form action="/api/user/addFavoritePokemon?_method=PUT&pokeId=<%= pokemon._id %>" method="post">
            <input type="submit" name="_method" value="Favorite Me!" />
        </form>
    <% } %>
